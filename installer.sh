...
REPO="https://github.com/itzNightmare17/Oreo.git"
CURRENT_DIR="$(pwd)"
ENV_FILE_PATH=".env"
DIR="/root/itzNightmare17"

while [ $# -gt 0 ]; do
    case "$1" in
    --dir=*)
        DIR="${1#*=}" || DIR="/root/itzNightmare17"
        ;;
    --branch=*)
...
clone_repo() {
    # check if pyoreo, startup, plugins folders exist
    cd $DIR
    if [ -d $DIR ]; then
        if [ -d $DIR/.git ]; then
            echo -e "Updating Oreo ${BRANCH}... "
            cd $DIR
            git pull
...
main() {
    echo -e "Starting Oreo Setup..."
    if [ -d "pyOreo" ] && [ -d "resources" ] && [ -d "plugins" ]; then
        DIR=$CURRENT_DIR
    fi
    if [ -f $ENV_FILE_PATH ]
    then
